	...
	for (iter = 0; iter < num_epochs_CNN; iter++) {
		...
		for (int i = 0; i < num_patterns_train_CNN; i++) {
			...
			if (i % 1000 == 0) {
				...
				printf("%dth --> forward: %1ld ms, ", i, t1Duration);
				gettimeofday(&tsBegin, NULL);
			}

			//2 Output Error for Back Propagation
			...
			if (i % 1000 == 0) {
				gettimeofday(&tsEnd, NULL);
				t1Duration = 1000000L * (tsEnd.tv_sec - tsBegin.tv_sec) + (tsEnd.tv_usec - tsBegin.tv_usec);
				printf("backward: %1ld ms, ", t1Duration);
				gettimeofday(&tsBegin, NULL);
			}
			...
			if (i % 1000 == 0) {
				gettimeofday(&tsEnd, NULL);
				t1Duration = 1000000L * (tsEnd.tv_sec - tsBegin.tv_sec) + (tsEnd.tv_usec - tsBegin.tv_usec);
				printf(" UpdateWeights: %1ld ms", t1Duration);
